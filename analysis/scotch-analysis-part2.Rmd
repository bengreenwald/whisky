...
```{r, include=FALSE}
# pull specific whisky prices for dynamic inline code
...
[Code for Part 2](GITHUB URL) available on Github.

## Next Steps
We've compared variables such as `age`, `ABV`, `price`, and `points` and learned about whisky `type` from scotch sourced all across Scotland. Next, [Part 2](PAGE/GITHUB LINK) focuses on the regionality of scotch to better understand the distribution and clustering of distilleries across different regions of Scotland -- and what makes each region's whisky special and unique.
